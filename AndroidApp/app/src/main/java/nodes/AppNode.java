...
public class AppNode extends Node {
    ...
    public Video loadIndex() throws IOException {
        System.out.println("Reading files from uploadDirectory: " + uploadDirectory);

        return index.loadFromDisk(uploadDirectory);
    }
    ...
    public Video pull(String topic, String videoName) throws IOException {
        String username = topic.split(":")[0];
        ...
        if (x == 0) {
            ...
            File parent = new File(downloadDirectory + File.separator + username + File.separator + channel);
            parent.mkdirs();

            File f = new File(downloadDirectory + File.separator + username + File.separator + channel + File.separator + videoName);

            Files.write(bytes, f);

            System.out.println("Transfer complete.");

            Video video = new Video(0, videoName, f.getAbsolutePath(), topic);
            return video;
        }
    }
    ...
    public Index getDownloads() {
        ...
        try {
            index.loadFromDownloadsDisk(downloadDirectory);
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public Index getAvailableList() {
        ...
        try {
            ...
            for (Map.Entry<BrokerData, Client> entry : clients.entrySet()) {
                ...
                Set<String> clientchannels = client.receiveChannels();

                channels.addAll(clientchannels);
            }
            ...
            for (String channel : channels) {
                ...
                System.out.println("----------");
                for (Video v : videos) {
                    if (!v.topic.startsWith(username)) {
                        index.add(v);
                    }
                }
            }
